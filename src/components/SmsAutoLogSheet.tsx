      ...
      {(close) => (
        <>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">SMS auto-logging</h2>
            <button
              onClick={close}
          ...
          {webhookUrl ? (
            <>
              <label className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">Your private webhook link</label>
              <div className="mb-2 flex items-center gap-2">
                <input
                  readOnly
                  value={webhookUrl}
                  onFocus={(e) => e.target.select()}
                  className="w-full min-w-0 truncate rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-600 outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
                />
                <button
                  onClick={copy}
              ...
              <p className="mb-2 text-xs font-medium text-slate-500 dark:text-slate-400">Set up in iOS Shortcuts</p>
              <ol className="mb-2 list-decimal space-y-1.5 pl-4 text-xs text-slate-600 dark:text-slate-400">
              ...
              <p className="mb-1.5 mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">
                If your bank never mentions the card's last 4 digits
              </p>
              <p className="mb-2 text-xs text-slate-600 dark:text-slate-400">
                In step 4, use Request Body: JSON instead of Text, with two fields — <code className="rounded bg-slate-100 px-1 py-0.5 dark:bg-slate-700">message</code> set
                to the message text and <code className="rounded bg-slate-100 px-1 py-0.5 dark:bg-slate-700">sender</code> set to the trigger's Sender
